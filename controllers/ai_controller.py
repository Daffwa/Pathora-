...
@bp.get("/api/assistant/health", endpoint="api_assistant_health")
...
@bp.post("/api/assistant", endpoint="api_assistant")
def api_assistant():
    auth_error = _require_authenticated_user()
    if auth_error is not None:
        return auth_error
...
@bp.route("/ai-assistant/chat", methods=["POST"], endpoint="ai_assistant_chat")
