...
@bp.route("/uploads/avatars/<path:filename>", endpoint="profile_avatar")
def profile_avatar(filename):
    ...
    return send_from_directory(current_app.config["AVATAR_UPLOAD_FOLDER"], safe_filename)
...
@bp.route("/profile/edit", methods=["GET", "POST"], endpoint="edit_profile")
@jobseeker_required_decorator
def edit_profile():
    user = get_current_user_profile()
    if user is None:
