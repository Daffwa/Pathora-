...
from flask import Blueprint, abort, flash, redirect, render_template, request, send_from_directory, session, url_for, current_app
...
@bp.route("/documents/<path:doc_type>/update", methods=["POST"], endpoint="update_document")
@jobseeker_required_decorator
def update_document(doc_type):
    ...
    if uploaded_file and uploaded_file.filename:
        ...
        if existing_document and existing_document.file_path != saved_file_name:
            delete_file_if_exists(current_app.config["UPLOAD_FOLDER"], existing_document.file_path)
...
@bp.route("/documents/<path:doc_type>/reset", methods=["POST"], endpoint="reset_document")
@jobseeker_required_decorator
...
@bp.route("/documents/<path:doc_type>/download", endpoint="download_document")
@jobseeker_required_decorator
