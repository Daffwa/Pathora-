...
| Avatar profile | `/uploads/avatars/<filename>` | Pemilik avatar login dan akun aktif | Menampilkan file avatar milik user yang sedang login dari folder upload. | Tersedia |
...
| Download/view document | `/documents/<doc_type>/download` | Student / jobseeker | Mengunduh file dokumen yang sudah diupload sebagai attachment. | Tersedia |
...
| S-12 | Mengelola profile user | Student / jobseeker | Melengkapi identitas dan data pendukung rekomendasi. | Student sudah login. | Buka `/profile`.<br>Klik Edit Profil.<br>Ubah data pribadi, akademik, karier, skill, link, avatar, atau password.<br>Klik Simpan Perubahan. | Profile diperbarui dan kelengkapan profile dapat meningkat. | Tabel `users`, folder `uploads/avatars`. | Sistem memvalidasi nama/email wajib, email unik, tanggal lahir, IPK 0.00-4.00, semester 1-14, tahun masuk, password baru minimal 8 karakter, serta avatar jpg/jpeg/png dengan MIME type dan signature file yang sesuai. |
...
| Bookmark duplikat | User menyimpan peluang yang sama lebih dari sekali. | Unique constraint `(user_id, opportunity_id)` memicu error integritas SQLAlchemy, lalu muncul pesan peluang sudah ada di Saved Opportunities. | Tersedia |
...
| Database gagal diproses | Operasi insert/update/delete terkena error SQLAlchemy atau koneksi database. | Beberapa route menampilkan pesan gagal seperti "Silakan coba lagi." | Tersedia |
...
**OOP.** Proyek memakai SQLAlchemy ORM pada folder `models` untuk merepresentasikan tabel database sebagai class, serta `dataclass` pada folder `dto` untuk kebutuhan view model seperti `Opportunity` dan `Document`.
...
**Error handling.** Aplikasi menangani error melalui validasi form, `try-except` untuk database, error integritas SQLAlchemy untuk duplikasi bookmark/tracker, handler 404 dan 403, serta handler `RequestEntityTooLarge` untuk file lebih dari 5 MB. Pesan error ditampilkan ke user memakai `flash`.

**Modularity.** Struktur proyek cukup modular: `app.py` untuk application factory, `config.py` untuk konfigurasi, `models` untuk SQLAlchemy ORM, `repositories` untuk akses database, `services` untuk business logic, `controllers`/`routes` untuk request flow, `migrations` untuk schema Alembic, `templates` untuk tampilan Jinja, `static` untuk CSS/JavaScript, dan `uploads` untuk file user.
